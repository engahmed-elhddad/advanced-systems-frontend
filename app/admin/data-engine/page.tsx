...
export default function DataEnginePage() {
  ...
  const fetchData = () => {
    Promise.all([
      fetch(`${API}/admin/data-engine/status`, { headers: { "api-key": ADMIN_KEY } })
        .then((r) => r.json())
        .catch(() => ({})),
      fetch(`${API}/admin/data-engine/logs?limit=80`, { headers: { "api-key": ADMIN_KEY } })
        .then((r) => r.json())
        .then((d) => d.logs || [])
        .catch(() => []),
    ]).then(([s, l]) => {
  ...
  const runJob = async (jobId: string) => {
    setRunning(jobId)
    try {
      const res = await fetch(`${API}/admin/data-engine/run/${jobId}`, {
        method: "POST",
