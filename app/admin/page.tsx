...
const STATUS_COLORS: Record<string, string> = {
  ...
  responded: "bg-primary-100 text-primary-700 border-primary-200",
...
export default function AdminDashboard() {
  ...
  const statCards = stats
    ? [
        { label: "Products", value: stats.total_products, color: "text-primary-600", bg: "bg-primary-50" },
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Industrial Automation Marketplace — System Control</p>
          </div>
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 font-medium text-sm transition-colors"
          >
            ← Back to Site
          </Link>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-10">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 skeleton" />
            ))}
          </div>
        ) : stats ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-10">
                  ...
                  {section.links.map((link) => (
                    <li key={link.href}>
                      ...
                      >
                        ...
                        <span className="text-gray-400 ml-2">— {link.desc}</span>
                      </Link>
                    </li>
                  ))}
                </ul>
              ...
              <tbody>
                ...
                {recentRFQs.map((r) => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition">
                    ...
                    <td className="px-6 py-3 font-mono text-primary-700">{r.part_number}</td>
                    <td className="px-6 py-3 text-gray-600">{r.email}</td>
                    <td className="px-6 py-3">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${STATUS_COLORS[r.status] ?? "bg-gray-100 text-gray-600 border-gray-200"}`}
                      >
                        {r.status}
                      </span>
                    </td>
