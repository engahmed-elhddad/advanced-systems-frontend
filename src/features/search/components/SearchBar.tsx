...
let cachedLookupData: { brands: BrandItem[]; categories: CategoryItem[] } | null = null
...
export interface SearchBarProps {
  ...
  brandPath?: string
  /** Category page path (default /categories) */
  categoryPath?: string
}
...
export function SearchBar({
  ...
  useEffect(() => {
    if (cachedLookupData) {
      setAllBrands(cachedLookupData.brands)
      setAllCategories(cachedLookupData.categories)
      return
    }

    let cancelled = false
    Promise.all([
      apiFetch(`${API_BASE}/api/v1/brands/`).then((r) => r.json()),
      apiFetch(`${API_BASE}/api/v1/categories/`).then((r) => r.json()),
    ]).then(([brandData, catData]) => {
      if (!cancelled) {
        const brands = normalizeBrands(brandData)
        const categories = normalizeCategories(catData)
        cachedLookupData = { brands, categories }
        setAllBrands(brands)
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await apiFetch(
          `${API_BASE}/api/v1/search/autocomplete?q=${encodeURIComponent(query.trim())}&limit=${Math.max(5, suggestionLimit)}`
        )
  ...
  const filteredBrands = useMemo(() => {
    if (qNorm.length < minLength) return []
    return allBrands
      .filter((b) => b.name.toLowerCase().includes(qNorm) || b.slug.toLowerCase().includes(qNorm))
      .slice(0, 4)
  }, [allBrands, qNorm, minLength])

  const filteredCategories = useMemo(() => {
    if (qNorm.length < minLength) return []
    return allCategories
      .filter((c) => c.name.toLowerCase().includes(qNorm) || c.slug.toLowerCase().includes(qNorm))
      .slice(0, 4)
  }, [allCategories, qNorm, minLength])

  const productOptions = useMemo(
    () =>
      productSuggestions.slice(0, 5).map((s) => ({
  ...
  const brandOptions = useMemo(
    () => filteredBrands.map((b) => ({ type: 'brand' as const, name: b.name, slug: b.slug })),
    [filteredBrands]
  )
  ...
  const options: SuggestionOption[] = useMemo(
    ...
    [productOptions, brandOptions, categoryOptions, hasViewAll]
  )
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      if (!dropdownOpen) {
        if (e.key === 'ArrowDown' && options.length > 0 && query.trim().length >= minLength) {
          e.preventDefault()
          setDropdownOpen(true)
          setActiveIndex(0)
          return
    ...
    [dropdownOpen, activeIndex, totalOptions, options, selectOption, navigateToSearch, query, minLength]
  )
