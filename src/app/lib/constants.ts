...
export const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL || "https://www.advancedsystems-int.com"
...
/** Map category URL/query text to `categories.slug` for `/api/v1/products` and `/api/v1/search` (hyphenated, lowercase). */
export function normalizeCategoryQueryForApi(input: string): string {
  const t = (input ?? "").trim()
  if (!t) return ""
  const lower = t.toLowerCase()
  const byName = CATEGORIES.find((c) => c.name.toLowerCase() === lower)?.slug
  if (byName) return byName
  const bySlug = CATEGORIES.find((c) => c.slug === lower)?.slug
