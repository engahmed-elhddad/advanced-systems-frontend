  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden hover:border-primary-200 transition-colors">
      <div className="flex flex-col sm:flex-row">
        <Link href={`/products/${encodeURIComponent(p.part_number)}`} className="block relative w-full sm:w-28 h-36 sm:h-28 bg-slate-50 shrink-0">
          <Image
            src={imgSrc}
            alt={p.part_number}
            fill
        ...
        <div className="flex-1 p-4 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <Link href={`/products/${encodeURIComponent(p.part_number)}`} className="font-mono font-semibold text-primary-600 hover:underline">
                {p.part_number}
              </Link>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4 shadow-sm">
                <span className="animate-pulse text-sm text-slate-500">Searching productsâ€¦</span>
              </div>
            </div>
          )}
        </div>
