...
import type { B2BOrderStatus, OrderDetail, OrderNote, OrderTimelineEvent } from '@/services/adminService'
...
export function OrderDetailPanel({
  ...
  if (!order) {
    return (
      <div className="rounded-[4px] border border-[#E5E7EB] bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-1 text-xs text-[#6B7280]">Items, actions, timeline, and notes will appear here.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
            ...
            <tbody>
              {order.items.map((item) => (
                <tr key={item.id} className="border-t border-[#F3F4F6]">
                  ...
                  <td className="px-3 py-2.5 text-sm font-semibold text-[#1A1A1A]">{formatCurrency(item.line_total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <OrderNotes notes={notes} loading={notesLoading} submitting={notePending} onAddNote={onAddNote} />
    </div>
  )
}
