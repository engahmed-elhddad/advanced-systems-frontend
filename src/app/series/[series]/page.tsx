...
import { API_BASE_URL, slugToCategory, categoryToSlug } from "@/lib/constants"
...
function SearchResults() {
  ...
  useEffect(() => {
    ...
    suggestDebounceRef.current = setTimeout(async () => {
      try {
        let res = await fetch(`${API}/api/v1/search/suggest?q=${encodeURIComponent(term)}&limit=8`)
        if (!res.ok) res = await fetch(`${API}/search/suggest?q=${encodeURIComponent(term)}&limit=8`)
  ...
  useEffect(() => {
    fetch(`${API}/api/v1/search/filter-options`)
      .then((r) => (r.ok ? r.json() : fetch(`${API}/search/filter-options`).then((x) => x.json())))
  ...
  useEffect(() => {
    fetch(`${API}/api/v1/search/filters`)
      .then((r) => (r.ok ? r.json() : fetch(`${API}/filters`).then((x) => x.json())))
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const v1p = new URLSearchParams(params)
        v1p.set("size", String(limit))
        let res = await fetch(`${API}/api/v1/search/?${v1p}`)
        if (!res.ok) {
          const leg = new URLSearchParams(params)
          leg.set("limit", String(limit))
          res = await fetch(`${API}/search?${leg}`)
        }
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/v1/search/suggest?q=${encodeURIComponent(q.trim())}&limit=5`)
      .then((r) => (r.ok ? r.json() : fetch(`${API}/search/suggest?q=${encodeURIComponent(q.trim())}&limit=5`).then((x) => x.json())))
      .then((data) => {
  ...
  function selectSuggestion(s: { part_number: string }) {
    setShowSuggestions(false)
    router.push(`/products/${encodeURIComponent(s.part_number)}`)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Sticky search bar â€“ industrial */}
          ...
          <form onSubmit={handleSearch} className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Search part number, brand, or descriptionâ€¦"
                  ...
                  <>
                    ...
                    {(seriesParam || voltageParam || currentParam || mountingParam) && (
                      <span className="ml-2 text-gray-500">
                        {[seriesParam, voltageParam, currentParam, mountingParam].filter(Boolean).join(" Â· ")}
                      </span>
                    )}
                  </>
                ) : (
