...
export function CartIcon({ open, onOpenChange }: CartIconProps) {
  ...
  const cartText = totalCount > 0 ? `Quote cart — ${totalCount} item${totalCount !== 1 ? 's' : ''}` : 'Quote cart'
  ...
  return (
    <button
      ...
      className="relative inline-flex h-8 items-center gap-2 rounded-md px-2 text-[--text-secondary] transition-colors hover:bg-white/[0.08] hover:text-[--text-primary]"
    >
      <ShoppingCart className="h-4 w-4" />
      <span className="hidden text-xs font-medium md:inline">{cartText}</span>
      {totalCount > 0 ? (
